---
name: daily
description: "明日方舟日常任务：基建收菜→信用商店→公招→会客室线索→（周六）剿灭→（理智>30）刷1-7"
tags: [日常, 任务, 每日, daily, routine, 编排]
game: arknights
type: orchestrator
verified: false
subskills:
  - base-collect
  - credit-shop
  - recruit
  - annihilation
  - farm-1-7
verify: daily-reward-check
---

## Steps

0. 🔴 **必须先回到主界面！** 不管当前在哪个界面（公招、基建、作战……），先用左上角主页图标或反复 adb_back 回到主界面。确认看到「基建」「采购中心」「公开招募」等主界面按钮后再开始执行以下步骤。**禁止在任何非主界面的画面上开始执行子任务。**
1. 进入基建后调用 `base_collect()` 工具 → 内置截图通知 → subtask_done('base-collect', ...)
   - 如果 base_collect() 返回 success: false → subtask_done('base-collect', '收菜失败：<error>')，继续下一个子任务
2. 回到主界面后调用 `credit_shop()` 工具 → 内置截图通知 → subtask_done('credit-shop', ...)
   - 如果 credit_shop() 返回 success: false → subtask_done('credit-shop', '失败：<error>')，继续下一个子任务
3. 回到主界面后调用 `recruit()` 工具 → 内置截图通知 → subtask_done('recruit', ...)
   - 如果 recruit() 返回 success: false → subtask_done('recruit', '失败：<error>')，继续下一个子任务
   - 如果结果中有跳过高资的栏位 → subtask_done('recruit', 'X个栏位已招募，X个跳过高资')
4. 检查 [系统上下文] 中的星期和理智：
   - 如果是周六或周日且剿灭未完成 → skill_run('annihilation')
   - 如果理智 > 30 → **执行两轮**：先 skill_run('farm-1-7')，等待完成后再 skill_run('farm-1-7')（共两轮6倍，消耗72理智）
   - 🔴 **条件不满足直接跳过**：跳过前先调用 subtask_done('<skill_name>', '条件不满足跳过')，例如 subtask_done('annihilation', '周四跳过')。这样系统知道该子任务已处理完毕。
   - 🔴 两轮刷完后，先去 Step 5 检查日常奖励。如果奖励还有未领完的档位 → 继续刷 1-7 补充每日任务点数，直到所有档位领取完毕。
5. 完成任务前验证（严格按顺序执行）：
   - ① 主界面 → 点「任务」→ 点「日常」tab（最左侧，0.15,0.11）→ 点「收集全部」
   - ② 🔴 **必须先清弹窗再截图**：确认 OCR 中没有任何弹窗文字后，才进入下一步。
   - ③ 🔴 OCR 中出现「报酬已领取」+ 弹窗已清 → notify_with_screen 截图。英文 REWARD RECEIVED / PLEASE WAIT 是水印不是弹窗，忽略。**只截一次**。
   - ④ 周日：切「周常」tab（0.35,0.11）→ 重复②③
   - ⑤ subtask_done('daily-reward-check', '已确认奖励并截图')
6. 🔴 调用 task_complete()。



## Pitfalls

- 剿灭每周六打，刷关是可选的，根据用户的理智和未完成任务需求决定
- 如果某个子技能执行失败（如基建加载超时），调用 notify_with_screen("子技能失败：<技能名>") 通知用户并继续执行后续技能，不要全部中断
- 🔴 **子技能成功完成后必须先 notify_with_screen 截图再 subtask_done**（base_collect、credit_shop、recruit 除外，这三个工具的截图通知由工具内部处理）。notify 截图 → subtask_done，顺序不能反
- 任务面板顶部 tab 从左到右：日常(adb_tap_position 0.15,0.11) | 周常(0.35,0.11) | 主线 | 特勤。不要误入特勤或主线！
- 🔴 **「报酬已领取」= 该档奖励已拿完，画面上是半透明水印，不是按钮也不是弹窗！** 看到这四个字说明奖励已领取完毕，不要点它、不要 adb_back 关它、不要围着它反复点。截图确认后直接 subtask_done。
- 🔴 **notify_with_screen 截图必须干净**：确认能看到任务列表本身再截图
- 🔴 "正在提交反馈至神经……"是服务器通信动画，领取还未完成。必须等它消失、画面稳定后再判断是否全部领取完毕
- 🔴 **不要滚动任务左侧奖励栏！** 已领取奖励自动沉底，只检查顶部就能判断是否全部领完。滚动到底部看到的全是已完成，会产生全部完成的错觉。
